...
import uuid
import hashlib
from lazydeveloper.ffmpeg import fix_thumb, take_screen_shot, add_watermark, add_intro_to_video
import random
...
async def terminate_task(bot, message):
    """Handle /terminate command to remove and stop a task."""
    try:
        user_id = message.from_user.id
        if len(message.command) < 2:
            await message.reply("❗ Usage: /terminate <task_id>")
            return
        ...
        # Remove task from active processing or queue
        if task_status == "Processing":
            # Decrement active tasks count
            user_tasks[user_id]["active"] -= 1
            # Interrupt the current task (optional: implement cancellation in `process_task`)
        elif task_status == "Queued":
            # Remove task from the queue
            queue = user_tasks[user_id]["queue"]
            new_queue = Queue()
            while not queue.empty():
                queued_task = await queue.get()
                if queued_task["id"] != task_id:  # Add tasks back except the one to terminate
                    await new_queue.put(queued_task)
            user_tasks[user_id]["queue"] = new_queue
        else:
            await message.reply(f"❗ Cannot terminate task with status: {task_status}")
            return

        # Remove task from task details
        del task_details[task_id]

        await message.reply(
            f"✅ Task with ID <code>{task_id}</code> has been successfully terminated!",
            parse_mode=enums.ParseMode.HTML
        )
        ...
        print(f"Error in terminate_task: {e}")
